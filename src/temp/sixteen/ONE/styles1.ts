...
/**
 * ScrollOuter
 * — scroll-snap-type: x mandatory  → snaps horizontally on every breakpoint
 * — on mobile (<992px) the sticky first col is 120px wide,
 *   plan cols fill the remaining viewport width (100vw - 120px - 32px padding)
 *   so exactly ONE plan is visible at a time.
 */
...
export const Table = styled.table`
  ...
  /* desktop: all columns visible naturally */
...
// ── thead columns ─────────────────────────────────────────────────────────────
/**
 * Th (label col + plan cols)
 *
 * First child  = sticky label column (narrower on mobile: 120px)
 * Other children = plan columns
 *   • mobile  → width = calc(100vw - 120px - 32px) so 1 plan fits the screen
 *   • desktop → natural min-width
 *   • scroll-snap-align: start on every plan column
 */
export const Th = styled.th`
  ...
  /* plan columns — snap target */
  scroll-snap-align: start;

  /* mobile: each plan column fills exactly the remaining viewport */
  width: calc(100vw - 120px - 32px);
  min-width: calc(100vw - 120px - 32px);

  @media (min-width: 992px) {
    width: auto;
    min-width: 190px;
    scroll-snap-align: none; /* let desktop scroll freely */
  }

  /* label column overrides */
  &:first-child {
    scroll-snap-align: none; /* label col never snaps */
    width: 120px;
    min-width: 120px;
    position: sticky;
    left: 0;
...
/**
 * ValueTd
 * — mirrors Th sizing so snap alignment is pixel-perfect:
 *   mobile → same calc() width as the plan Th
 *   desktop → natural min-width
 * — scroll-snap-align: start so each cell column snaps too
 */
